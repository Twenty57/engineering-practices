...
it's up to the judgment of your reviewer.
...
-   You can usually count deletion of an entire folders or functions as being just one line of
    change, because it doesn't take the reviewer very long to review.
-   Sometimes a large PR has been generated by refactoring names or moving things around, and the reviewer's job is just to verify and say
...
### Splitting by Dependency {#splitting-dependency}

Another way to split up a PR is by working on pieces of functionality that does
not depend on each other.

For example: you send off one PR for modifications to the email sending infrastructure
and then start to work on adding a new API endpoint.
...
For example: You created a calculator app with client, API, and data
model layers. An API stub can split the implementation of
client code from service code and enable them to move forward independently.
Similar ideas can also be applied to more granular abstractions.

## Separate Out Refactorings {#refactoring}

It's usually best to do refactorings in a separate PR from feature changes or
bug fixes. For example, moving and renaming a function should be in a different PR
from fixing a bug in that function. It is much easier for reviewers to understand
