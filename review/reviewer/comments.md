...
Bad: "Why did **you** use a try-catch here when there's obviously no benefit to be
gained from it?"

Good: "This is hiding potential errors without any
benefit that I can see. Because there's no benefit, it's best for this code to rather
throw an exception if something goes wrong instead of swallowing the error in a try-catch.
The earlier we fail when unexpected things happen, the closer we are to the origin of the
error and the easier it is to find and fix.
